...
func hurt():
	if life != 0:
		...
		$AnimationTree["parameters/playback"].travel("head_movement")
		$HurtTimer.start()
		rot_speed = 0.05
		harass = false
		player_escape = true
		...
		harass = false
		player_escape = true
		$AnimationTree["parameters/playback"].travel("power_off")
		$HurtTimer.start()
		$Dead_Timer.start()
...
func _on_Hitbox_body_entered(body):
	if body.name == ("Character1"):
		...
		$AnimationPlayer.play("spit_attack")
		$AnimationTree["parameters/playback"].travel("spit_attack")
		print("bitten")
